...
export const getAllIgPosts = async () => {
  const access_token: string = import.meta.env.IG_ACCESS_TOKEN as string;
  const url =
    // eslint-disable-next-line max-len
    'https://graph.facebook.com/v17.0/17841460325620436?fields=business_discovery.username(tagomagoband){media{caption,timestamp,permalink,media_type,media_url,children{media_url,media_type}}}&access_token=' +
    access_token;
  const mediaData: Post[] = [];
  const response = await fetch(url);
  const data = await response.json();
  if (!data.error) {
    data.business_discovery.media.data.map((post: Post) => {
      mediaData.push(post);
    });
    return mediaData;
  } else {
